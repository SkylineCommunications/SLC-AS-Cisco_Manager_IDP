...
public class Script
{
	...
	private string GetDeviceFullBackupAsText(IActionableElement element)
	{
		...
		element.SetParameter(42011, fileType);  // Copy Config Source File Type (1: Network file, 3: Startup config, 4: running config)
		element.SetParameter(42012, 1);         // Copy Config Destination File Type (1: Network file, 3: Startup config, 4: running config)
		...
		var backup = new BackupDataFileType
		{
			Tftp = tFtp,
			FileType = fileType,
			FileName = fileName,
